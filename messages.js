...
function filtered() {
  ...
  return messages.filter(m => {
    const catOk =
      ...
      m.category === state.category;

    const textOk =
      !term ||
      `${m.title} ${m.text} ${m.category}`
        .toLowerCase()
        .includes(term);
...
function renderMessages() {
  ...
  // 64 mensagens
  countEl.textContent =
    `${list.length} ${list.length === 1 ? "mensagem" : "mensagens"}`;
  ...
  messagesEl.innerHTML = list.map((m, i) => `
    <article class="card">
      ...
      >${escapeHtml(m.text)}</textarea>

      <div class="actions">
        <button class="copy" data-index="${i}">
          📋 Copiar
        </button>
      </div>
    </article>
  ...
  messagesEl.querySelectorAll(".copy").forEach(btn => {
    btn.addEventListener("click", async () => {
      const textarea = messagesEl.querySelector(
        `#msg-${btn.dataset.index}`
      );
